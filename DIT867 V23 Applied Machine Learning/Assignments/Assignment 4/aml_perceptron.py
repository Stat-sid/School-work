...
class SVC(LinearClassifier):
    def __init__(self, n_iter=20) -> None:
        self.n_iter = n_iter
    ...
    def fit(self, X, Y, lambda_value=0.001):
        ...
        for i in range(self.n_iter):
            for x, y in zip(X, Ye):
                ...
                eta = 1/(lambda_value * t)
                score = self.w @ x 
                # score = np.matmul(self.w, x)

                if (y * score) < 1:
                    self.w = (1-eta * lambda_value) * self.w + (eta*y) * x
                else:
                    self.w *= (1-eta * lambda_value)


class LogReg(LinearClassifier):
    def __init__(self, n_iter=20) -> None:
        self.n_iter = n_iter
