...
FROM taken AS t left outer join courses AS c on code = course ; 
...
SELECT StudentBranches.student as student, SUM(COALESCE(PassedCourses.credits,0)) as reccredits
	...
	thelp AS (SELECT basicInformation.idnr AS student1, COALESCE(SUM(passedCourses.credits), 0) AS totalCredits1
    FROM basicInformation
	inner JOIN passedCourses on passedCourses.student = idnr
	GROUP BY basicInformation.idnr
    )
    SELECT calculations.student,totalCredits1 as totalCredits, mandatoryLeft, mathCredits, researchCredits, seminarCourses,
    ...
    From calculations 
    LEFT JOIN calc ON calculations.student = calc.student
	LEFT JOIN thelp ON calculations.student = student1;