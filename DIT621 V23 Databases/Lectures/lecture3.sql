...
CREATE OR REPLACE VIEW Densities AS
  (SELECT name, abbr, ROUND(population/area) AS density FROM Countries);

CREATE OR REPLACE VIEW Densities AS
  (SELECT name, abbr AS density, ROUND(population/area) FROM Countries);
*/
...
-- Full cartesian product: contains many non interesting rows!
...
-- Selecting only the rows that make sense
SELECT Countries.name, code, Currencies.name, value
FROM Countries, Currencies
WHERE currency = code;
...
-- Will not work: the aliasing needs to be use in the select then
SELECT Countries.name, code, Currencies.name, value
FROM Countries AS Co, Currencies AS Cu
WHERE currency = code;
*/
...
SELECT *
FROM Capitals JOIN CurrencyCodes 
...
-- We insert a country with without currency
INSERT INTO CurrencyCodes VALUES ('France', NULL);
